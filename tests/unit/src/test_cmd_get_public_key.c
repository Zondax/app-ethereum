...
void getEthAddressFromRawKey(const uint8_t raw_pubkey[static 65],
    ...
    memset(out, 0xAA, ADDRESS_LENGTH);
}
...
void test_no_reset_when_idle_and_not_from_swap(void) {
    uint8_t apdu[64];
...
void test_skips_reset_when_called_from_swap(void) {
...
void test_derive_failure_rejected(void) {
...
void test_non_confirm_happy_path_writes_response(void) {
    uint8_t apdu[64];
    size_t len = build_apdu(apdu, sizeof(apdu), false, 0);
    unsigned int tx = 0;
    uint16_t sw = handle_get_public_key(P1_NON_CONFIRM, P2_NO_CHAINCODE, apdu, (uint8_t) len, &tx);
    ...
    TEST_ASSERT_EQUAL(tx, 1 + 65 + 1 + 40);
    TEST_ASSERT_EQUAL(G_io_tx_buffer[0], 65);
    TEST_ASSERT_EQUAL(G_io_tx_buffer[66], 40);
    // No UI on NON_CONFIRM.
    TEST_ASSERT_EQUAL(g_ui_display_calls, 0);
}

void test_chaincode_path_extends_response(void) {
    uint8_t apdu[64];
    size_t len = build_apdu(apdu, sizeof(apdu), false, 0);
    unsigned int tx = 0;
    ...
    TEST_ASSERT_EQUAL(tx, 1 + 65 + 1 + 40 + 32);
    // The chaincode bytes the wrap filled with 0xCC must land after
    // the address (offset 1 + 65 + 1 + 40 = 107).
    TEST_ASSERT_EQUAL(G_io_tx_buffer[107], 0xCC);
}

void test_confirm_calls_ui_and_returns_no_response(void) {
    uint8_t apdu[64];
    size_t len = build_apdu(apdu, sizeof(apdu), false, 0);
    unsigned int tx = 0;
...
void test_chain_id_mismatch_rejected_on_clone(void) {
...
void test_chain_id_match_accepted_on_clone(void) {
...
void test_mainnet_app_accepts_any_chain_id(void) {
...
void test_leftover_bytes_rejected(void) {
    uint8_t apdu[64];
    size_t len = build_apdu(apdu, sizeof(apdu), true, 1);
...
void test_get_public_key_happy_path(void) {
    uint8_t out[20];
    memset(out, 0, sizeof(out));
    uint16_t sw = get_public_key(out, sizeof(out));
    TEST_ASSERT_EQUAL(sw, SWO_SUCCESS);
    TEST_ASSERT_EQUAL(g_eth_addr_from_raw_calls, 1);
    // The wrap fills out with 0xAA.
    for (int i = 0; i < 20; i++) {
        TEST_ASSERT_EQUAL(out[i], 0xAA);
    }
}
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_no_reset_when_idle_and_not_from_swap);
    RUN_TEST(test_skips_reset_when_called_from_swap);
    RUN_TEST(test_invalid_p1_rejected);
    RUN_TEST(test_invalid_p2_rejected);
    RUN_TEST(test_bad_bip32_rejected);
    RUN_TEST(test_derive_failure_rejected);
    RUN_TEST(test_non_confirm_happy_path_writes_response);
    RUN_TEST(test_chaincode_path_extends_response);
    RUN_TEST(test_confirm_calls_ui_and_returns_no_response);
    RUN_TEST(test_chain_id_mismatch_rejected_on_clone);
    RUN_TEST(test_chain_id_match_accepted_on_clone);
    RUN_TEST(test_mainnet_app_accepts_any_chain_id);
    RUN_TEST(test_leftover_bytes_rejected);
    RUN_TEST(test_get_public_key_short_buffer_rejected);
    RUN_TEST(test_get_public_key_derive_failure_propagates);
    RUN_TEST(test_get_public_key_happy_path);
    return UNITY_END();
}
